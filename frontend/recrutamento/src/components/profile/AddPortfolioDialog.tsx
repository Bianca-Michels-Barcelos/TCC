...
interface AddPortfolioDialogProps {
  ...
  onSubmit: (data: PortfolioData) => Promise<void>;
}
...
export default function AddPortfolioDialog({ open, onOpenChange, onSubmit }: AddPortfolioDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMsg = err.response?.data?.message || 'Erro ao adicionar portfólio';
      setError(errorMsg);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>Adicionar Portfólio</DialogTitle>
          <DialogDescription>
            Adicione um projeto ou trabalho ao seu portfólio
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={loading}>
              {loading ? 'Adicionando...' : 'Adicionar Portfólio'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
